...
fn find_max<T: std::cmp::PartialOrd>(collection: &[T]) -> Option<&T> {
    let mut maxValue = collection.iter().nth(0).clone();
    for i in 1.. {
        if i >= collection.len() {
            break;
        }
        if collection.iter().nth(i)> maxValue {
            maxValue = collection.iter().nth(i).clone();
        }
    }
    maxValue
}
...
fn contains_value<T: std::cmp::PartialEq>(collection: &[T], value: &T) -> bool {
    for i in 0.. {
        ...
        if collection.iter().nth(i) == Some(value) {
            return true;
        }   
    }
    return false;
